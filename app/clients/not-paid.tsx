...
import { buildOutstandingClientRow } from "@/lib/dashboard-clients";
...
export default function NotPaidClientsScreen() {
      ...
      try {
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Unable to load outstanding clients right now."
        );
      } finally {
        if (options?.isRefreshing) {
          setRefreshing(false);
  ...
  const handleRefresh = useCallback(async () => {
    await loadClients({ isRefreshing: true });
  }, [loadClients]);
    ...
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        <View style={styles.header}>
          <Text style={styles.title}>Outstanding clients</Text>
          <Text style={styles.subtitle}>
            Everyone with unpaid invoices — send reminders or edit terms.
          </Text>
        </View>
